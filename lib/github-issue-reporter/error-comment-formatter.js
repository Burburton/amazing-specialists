...
function getSecretsRedaction() {
  if (secretsRedactionModule === null) {
    try {
      ...
      console.warn('[github-issue-reporter] Failed to load secrets-redaction module:', error.message);
...
async function formatErrorComment(errorReport, variant = null) {
  ...
  let safeErrorReport = errorReport;
  if (secrets && secrets.scrubErrorReport) {
    try {
      const redactionResult = await secrets.scrubErrorReport(errorReport, { source: 'github-issue-reporter' });
      safeErrorReport = redactionResult.success ? redactionResult.scrubbed : errorReport;
    } catch (error) {
      console.warn('[github-issue-reporter] Secrets redaction failed, using original:', error.message);
    }
  }
