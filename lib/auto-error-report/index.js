...
let secretsRedactionModule = null;
...
function getSecretsRedaction() {
  if (secretsRedactionModule === null) {
    try {
      secretsRedactionModule = require('../secrets-redaction/index.js');
    } catch (error) {
      console.warn('[auto-error-report] Failed to load secrets-redaction module:', error.message);
      secretsRedactionModule = false;
    }
  }
  return secretsRedactionModule || null;
}

async function scrubErrorReport(errorReport, config) {
  const secrets = getSecretsRedaction();
  
  if (!secrets || !secrets.scrubErrorReport) {
    return errorReport;
  }
  
  const shouldRedact = config.privacy?.redact_secrets !== false;
  ...
  try {
    const redactionResult = await secrets.scrubErrorReport(errorReport, { source: 'auto-error-report' });
    return redactionResult.success ? redactionResult.scrubbed : errorReport;
  } catch (error) {
    console.warn('[auto-error-report] Secrets redaction failed, using original:', error.message);
    return errorReport;
  }
}
...
async function autoReportError(errorReport, options = {}) {
  ...
  const safeErrorReport = await scrubErrorReport(errorReport, config);
  
  try {
    const result = await reporter.reportToIssue(safeErrorReport, {
