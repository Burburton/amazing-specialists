...
### 其他 Skills（Legacy Transition - Bootstrap兼容）
- **spec-writer** - 产品规格编写（legacy，将迁移到 architect + docs）
- **architect-auditor** - 架构审计（legacy，将迁移到 architect + reviewer）
- **task-executor** - 任务执行器（legacy，将迁移到 developer/tester/docs/security）

> 这些 skills 属于 **3-skill 过渡骨架**，用于支撑早期 bootstrap 流程，不是最终角色体系。优先调用 6-role skills。
...
> **当前进度**: `003-architect-core`、`004-developer-core`、`005-tester-core`、`006-reviewer-core` 和 `007-docs-core` 已完成。architect、developer、tester、reviewer 和 docs 角色已成为拥有核心能力的 6-role 正式角色。后续 008 将实现 security 核心能力。
...
├── tester/              # 3个测试员技能（✅ 005-tester-core 正式实现）
├── reviewer/            # 3个审查员技能（✅ 006-reviewer-core 正式实现）
├── docs/                # 2个文档员技能（✅ 007-docs-core 正式实现）
├── security/            # 2个安全员技能（待 008-security-core 实现）
