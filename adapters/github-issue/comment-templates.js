...
### Recommendation
${Array.isArray(result.recommendation) ? result.recommendation.join(', ') : result.recommendation || 'CONTINUE'}
...
*Generated by GitHub Issue Adapter at ${new Date().toISOString()}*`;
  ...
  _formatArtifacts(artifacts) {
    ...
    return artifacts.map(a => `- ${a.artifact_type || a.type}: ${a.path}`).join('\n');
  }
